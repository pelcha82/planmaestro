...
import EmailVerification from "@/react-app/pages/EmailVerification";
...
export default function Home() {
  ...
  const sendVerificationCode = async (email: string) => {
    ...
    try {
      console.log('📤 [REGISTRO] Registrando usuario en base de datos');
      
      // Call the new database-based registration endpoint
      ...
      const result = await response.json();
      console.log('✅ [REGISTRO] Usuario registrado y código enviado:', result);
      return true;
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Set the actual error - ErrorDisplay will make it user-friendly
  ...
  // If verification should be shown, render it instead
  if (showVerification) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 px-4 py-8">
      <div className="w-full max-w-2xl">
        ...
        >
          {/* Progress Steps */}
          <ProgressSteps currentStep={1} steps={REGISTRATION_STEPS} />

          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-2 text-slate-800 tracking-tight">
            Crear Cuenta
          </h2>
          <p className="text-center text-slate-600 text-sm mb-6 sm:mb-8">
            Comienza ingresando tu correo electrónico
          </p>
          
          {/* Use ErrorDisplay component for user-friendly error messages */}
          {error && (
            <ErrorDisplay 
              error={error} 
              onRetry={handleRetry}
